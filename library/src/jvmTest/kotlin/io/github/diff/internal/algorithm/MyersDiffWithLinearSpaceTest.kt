...
internal class MyersDiffWithLinearSpaceTest {
        ...
        val expected =
            listOf(
                ...
                Delta(
                    DeltaType.DELETE,
                    Chunk(0, listOf("A")),
                    Chunk(1, emptyList<String>()),
                ),
                Delta(
                    DeltaType.DELETE,
                    Chunk(2, listOf("C")),
                    Chunk(2, emptyList<String>()),
                ),
                Delta(
                    DeltaType.DELETE,
                    Chunk(5, listOf("B")),
    ...
    fun testDiffMyersExample1ForwardWithListener() {
        ...
        val patch =
            helper.generate(
                original,
                revised,
                MyersDiffWithLinearSpace<String>(),
    ...
    @Ignore("Performance heavy test - optionally enable manually")
    @Test
    fun testPerformanceProblemsIssue124() {
        val old = listOf("abcd")
        val newl = (0 until 90_000).map { it.toString() }
        val diff = helper.generate(old, newl, MyersDiffWithLinearSpace<String>())
        assertTrue(diff.getDeltas().isNotEmpty())
    }
}
