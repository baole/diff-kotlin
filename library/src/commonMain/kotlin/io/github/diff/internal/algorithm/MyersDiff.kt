/*
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package io.github.diff.internal.algorithm

import io.github.diff.Change
import io.github.diff.DeltaType
import io.github.diff.DiffAlgorithm
import io.github.diff.DiffAlgorithmListener

/**
 * A clean-room implementation of Eugene Myers greedy differencing algorithm.
 */
internal class MyersDiff<T>(
    val equalizer: (T, T) -> Boolean,
) : DiffAlgorithm<T> {
    /**
     * {@inheritDoc}
     *
     * Return empty diff if get the error while procession the difference.
     */
    override fun computeDiff(
        source: List<T>,
        target: List<T>,
        progress: DiffAlgorithmListener?,
    ): List<Change> {
        progress?.diffStart()
        val path = buildPath(source, target, progress)
        val result = buildRevision(path, source, target)
        progress?.diffEnd()
        return result
    }

    /**
     * Computes the minimum diffpath that expresses de differences between the
     * original and revised sequences, according to Gene Myers differencing
     * algorithm.
     *
     * @param orig The original sequence.
     * @param rev The revised sequence.
     * @return A minimum [MyersDiffPathNode] Path accross the differences graph.
     * @throws IllegalStateException if a diff path could not be found.
     */
    private fun buildPath(
        orig: List<T>,
        rev: List<T>,
        progress: DiffAlgorithmListener?,
    ): MyersDiffPathNode {
        val origSize = orig.size
        val revSize = rev.size

        val maxSize = origSize + revSize + 1
        val size = 1 + 2 * maxSize
        val middle = size / 2
        val diagonal = arrayOfNulls<MyersDiffPathNode>(size)

        diagonal[middle + 1] = MyersDiffPathNode(0, -1, true, true, null)
        for (d in 0 until maxSize) {
            progress?.diffStep(d, maxSize)
            var k = -d
            while (k <= d) {
                val kmiddle = middle + k
                val kplus = kmiddle + 1
                val kminus = kmiddle - 1
                val prev: MyersDiffPathNode
                var i: Int

                if ((k == -d) || (k != d && diagonal[kminus]!!.i < diagonal[kplus]!!.i)) {
                    i = diagonal[kplus]!!.i
                    prev = diagonal[kplus]!!
                } else {
                    i = diagonal[kminus]!!.i + 1
                    prev = diagonal[kminus]!!
                }

                diagonal[kminus] = null // no longer used

                var j = i - k

                var node = MyersDiffPathNode(i = i, j = j, snake = false, bootstrap = false, prev = prev.previousSnake())

                while (i < origSize && j < revSize && equalizer(orig[i], rev[j])) {
                    i++
                    j++
                }

                if (i != node.i) {
                    node = MyersDiffPathNode(i = i, j = j, snake = true, bootstrap = false, prev = node)
                }

                diagonal[kmiddle] = node

                if (i >= origSize && j >= revSize) {
                    return diagonal[kmiddle]!!
                }
                k += 2
            }
            diagonal[middle + d - 1] = null
        }
        // According to Myers, this cannot happen
        throw IllegalStateException("could not find a diff path")
    }

    /**
     * Constructs a list of changes from a difference path.
     *
     * @param actualPath The path.
     * @param orig The original sequence.
     * @param rev The revised sequence.
     * @return A list of changes corresponding to the path.
     * @throws IllegalStateException if changes could not be built from the given path.
     */
    private fun buildRevision(
        actualPath: MyersDiffPathNode,
        orig: List<T>,
        rev: List<T>,
    ): List<Change> {
        var path: MyersDiffPathNode? = actualPath
        val changes = mutableListOf<Change>()
        if (path!!.isSnake()) {
            path = path.prev
        }
        while (path != null && path.prev != null && path.prev!!.j >= 0) {
            if (path.isSnake()) {
                throw IllegalStateException("bad diffpath: found snake when looking for diff")
            }
            val i = path.i
            val j = path.j

            path = path.prev
            val ianchor = path!!.i
            val janchor = path.j

            when {
                ianchor == i && janchor != j -> changes.add(Change(DeltaType.INSERT, ianchor, i, janchor, j))
                ianchor != i && janchor == j -> changes.add(Change(DeltaType.DELETE, ianchor, i, janchor, j))
                else -> changes.add(Change(DeltaType.CHANGE, ianchor, i, janchor, j))
            }

            if (path.isSnake()) {
                path = path.prev
            }
        }
        return changes
    }
}
